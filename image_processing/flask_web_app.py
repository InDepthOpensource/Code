...
def handle_inpaint_request():
        ...
        with torch.no_grad():
            ...
            start = time.time()
            _, completed_depth_buffer = cv2.imencode('.png', (pred[0][0] * 1000).astype(np.uint16))
            completed_depth_file = BytesIO(completed_depth_buffer)
            # Deprecate image io as it is too slow.
            # imageio.imwrite(completed_depth_file, (pred[0][0] * 1000).astype(np.uint16), format='.png', optimize=False,
            #                 compress_level=3)
            end = time.time()
        ...
        completed_depth_file.seek(0)
        return send_file(completed_depth_file, attachment_filename=depth_file.filename, as_attachment=True)
